...
/** Same query shape as get posts, with a higher default limit for download queue / bulk flows. */
export const GetDownloadItemsSchema = GetPostsSchema.extend({
  limit: z.number().int().min(1).max(500).default(500),
});

export type GetDownloadItemsRequest = z.infer<typeof GetDownloadItemsSchema>;

export const GetPostsCountWithFiltersParamsSchema = GetPostsSchema.pick({
  artistId: true,
  filters: true,
});

export type GetPostsCountWithFiltersParams = z.infer<
  typeof GetPostsCountWithFiltersParamsSchema
>;

export const MarkViewedIpcSchema = z.tuple([
  z.number().int(),
  PostDataSchema.optional(),
]);

export type MarkViewedIpcArgs = z.infer<typeof MarkViewedIpcSchema>;

export const ToggleFavoriteIpcSchema = z.tuple([
  z.number().int(),
  PostDataSchema.optional(),
]);

export type ToggleFavoriteIpcArgs = z.infer<typeof ToggleFavoriteIpcSchema>;

export const ResetPostCacheIpcSchema = z.tuple([z.number().int().positive()]);
