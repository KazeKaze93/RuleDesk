...
import type { AppDatabase } from "../core/services";
import {
  type PostData,
...
export type { AppDatabase } from "../core/services";

type GetPostsParams = GetPostsRequest;

/** Result row shape returned by getPosts (explicit select list). */
export type PostsListRow = {
  id: number;
  postId: number;
  artistId: number;
  fileUrl: string;
...
/**
 * Post database access and domain logic (FTS, favorites, shadow insert, etc.).
 * Used by PostsService for IPC; inject `AppDatabase` for tests.
 */
export class PostsService {
  ...
  private static readonly inFlightShadowInserts = new Map<
    string,
    Promise<InferSelectModel<typeof posts>>
  >();
  ...
  /**
   * Call once when wiring the app (e.g. from controller.setup) so tag filters use FTS when available.
   */
  public initializeFtsTableCheck(): void {
    try {
      ...
      log.info(
        `[PostsService] FTS table check initialized: ${this.ftsTableExistsCache}`
      );
    } catch (error) {
      log.warn(
        "[PostsService] Failed to check FTS table existence, using LIKE fallback:",
        error
      );
  ...
  private createTagFilterCondition(tagFilter: string): SQL {
    ...
    if (ftsTableExists) {
      ...
      log.error(
        `[PostsService] FTS5 table does not exist for tag filtering. ` +
    ...
    if (filters?.aiFilter === "hide" || filters?.aiFilter === "only") {
      ...
      if (ftsTableExists) {
        ...
        if (!ftsQuery || ftsQuery.trim().length === 0) {
          log.warn(
            "[PostsService] Empty FTS5 query for AI filter, skipping filter condition"
          );
        ...
        log.warn(
          "[PostsService] FTS5 table not found, using slow LIKE fallback for AI filter"
        );
        const aiTagPatterns = [
          "%ai_generated%",
          "%ai-generated%",
   ...
   */
  public async getPosts(params: GetPostsParams): Promise<PostsListRow[]> {
    ...
    try {
      ...
      if (filters?.sinceTracking === true) {
        ...
        log.info(
          `[PostsService] Retrieved ${result.length} posts ${
      ...
      log.error("[PostsService] Failed to get posts:", error);
   ...
   */
  public async getPostsCount(params: GetPostsCountRequest): Promise<number> {
    try {
      const db = this.db;
      ...
      log.info(
        `[PostsService] Posts count: ${total} ${
          artistId ? `for artist ${artistId}` : "(all artists)"
        }`
      );
      return total;
    } catch (error) {
      log.error("[PostsService] Failed to get posts count:", error);
      return 0;
    }
  }
   ...
   */
  public async getPostsCountWithFilters(
    ...
    try {
      ...
      if (filters?.sinceTracking === true) {
        ...
        const total = result[0]?.value ?? 0;
        log.debug(`[PostsService] Posts count with filters: ${total}`);
        return total;
      }
      ...
      log.error("[PostsService] Failed to get posts count with filters:", error);
      return 0;
    }
  }
   ...
   */
  public async markViewed(postId: number, postData?: PostData): Promise<boolean> {
    try {
      const db = this.db;
      ...
      if (!postData && postId <= 0) {
        log.warn(
          `[PostsService] Cannot mark external post as viewed without postData. postId: ${postId}`
        );
        return false;
      }
      ...
      if (postData) {
        ...
        log.debug(
          `[PostsService] Post (postId: ${postData.postId}) marked as viewed in transaction`
        );
        return true;
      }
      ...
      if (postId > 0) {
        ...
        if (markViewedResult) {
          log.debug(`[PostsService] Post ${postId} marked as viewed in transaction`);
          return true;
        }
      }
      ...
      log.warn(
        `[PostsService] Post with id ${postId} not found. For external posts from Browse, postData must be provided.`
      );
      return false;
    } catch (error) {
      log.error("[PostsService] Failed to mark post as viewed:", error);
      return false;
    }
  }
   ...
   * @throws {Error} If update fails
   */
  public async resetPostCache(postId: number): Promise<boolean> {
    try {
      const db = this.db;
      ...
      log.info(
        `[PostsService] Post ${postId} cache reset (marked as not viewed)`
      );
      return true;
    } catch (error) {
      log.error("[PostsService] Failed to reset post cache:", error);
      return false;
    }
  }
   ...
   * @throws {Error} If database operation fails
   */
  public async toggleFavorite(postId: number, postData?: PostData): Promise<boolean> {
    try {
      const db = this.db;
      ...
      db.transaction((tx) => {
        ...
        if (existingPost) {
          ...
          log.debug(
            `[PostsService] Post ${existingPost.id} (postId: ${existingPost.postId}) favorite toggled in transaction`
          );
          ...
          if (!existingArtist) {
            ...
            log.debug(
              `[PostsService] Created placeholder artist ${targetArtistId} for external post in transaction`
            );
          }
          ...
          log.debug(
            `[PostsService] Created new post (postId: ${postData.postId}) and set as favorited in transaction`
          );
      ...
      if ("existingPostId" in result) {
        ...
        log.info(
          `[PostsService] Post ${r.existingPostId} (postId: ${r.existingPostPostId}) favorite toggled to ${r.newFavoriteState}`
        );
        ...
        log.info(
          `[PostsService] Post new (postId: ${r.newPostPostId}) favorite toggled to ${r.newFavoriteState}`
        );
        return r.newFavoriteState;
      }
    } catch (error) {
      log.error("[PostsService] Failed to toggle favorite:", error);
      // Re-throw original error to preserve stack trace and context
      throw error;
    }
  }
    ...
    try {
      ...
      if (apiKey && safeStorage.isEncryptionAvailable()) {
        try {
          ...
          log.warn("[PostsService] Failed to decrypt API Key.", e);
      ...
      log.error("[PostsService] Failed to get decrypted settings:", error);
      return null;
    }
  }
   ...
   */
  public async shadowInsertPost(request: {
    postId: number;
    provider: ProviderId;
  }): Promise<InferSelectModel<typeof posts>> {
    ...
    const existingPromise = PostsService.inFlightShadowInserts.get(dedupeKey);
    if (existingPromise) {
      log.debug(`[PostsService] Reusing in-flight shadow insert for ${dedupeKey}`);
    ...
    PostsService.inFlightShadowInserts.set(dedupeKey, insertPromise);
    
    // Clean up after Promise resolves/rejects (prevent memory leak)
    insertPromise
      .finally(() => {
        PostsService.inFlightShadowInserts.delete(dedupeKey);
      })
      .catch(() => {
  ...
  private async performShadowInsert(
    ...
    log.info(`[PostsService] Shadow insert attempt: postId=${request.postId}, provider=${request.provider}`);
    ...
    try {
      ...
      if (existingPost) {
        log.debug(`[PostsService] Shadow insert: Post already exists (id: ${existingPost.id}, postId: ${request.postId})`);
        return existingPost;
      }
      ...
      db.transaction((tx) => {
        ...
        insertedPost = result;
        log.info(
          `[PostsService] Shadow insert: Post ${insertedPost.id} (postId: ${request.postId}) - ${insertedPost.createdAt.getTime() === now.getTime() ? "created" : "updated"}`
        );
      });
      ...
      log.error(`[PostsService] Failed to shadow insert post (postId: ${request.postId}):`, {
        error: errorMessage,
        postId: request.postId,
        provider: request.provider,
      });
