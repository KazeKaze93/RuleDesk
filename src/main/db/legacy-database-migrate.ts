 ...
 * Checkpoint failure leaves legacy files untouched and returns without creating
 * newDbPath. Empty stub newDbPath from a prior soft failure is removed so retry works.
 */
export async function migrateLegacyDatabase(params: {
  ...
  if (fileFs.existsSync(newDbPath)) {
    ...
    // Empty stub after a prior failed/skipped legacy migrate — remove so rename can land.
    log.warn(
      `[DB] Removing uninitialized stub at ${newDbPath} to retry legacy migrate`
    );
    fileFs.rmSync(newDbPath, { force: true });
    fileFs.rmSync(`${newDbPath}-wal`, { force: true });
    fileFs.rmSync(`${newDbPath}-shm`, { force: true });
  }
