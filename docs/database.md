...
- **Smart-playlist empty-guard:** Before FTS `MATCH`, `PlaylistController` probes with `SELECT 1 FROM posts_fts LIMIT 1` (stops at the first row). Empty index → `sql\`1 = 0\``. No in-memory count cache. This probe reads the external-content table, not the inverted index — do not reuse it as an AI-filter guard.
...
- **AI filter during the bulk-sync window:** `PostsController` uses FTS `MATCH` for `aiFilter` hide/only only while `areRuntimeDroppableFtsTriggersPresent` is true (real `sqlite_master` trigger DDL, not an FTS content probe and not `SyncService.isSyncing` — incremental sync is `isSyncing` with triggers still live). While insert/update are dropped, MATCH on never-indexed rows would fail-open hide (`NOT EXISTS(MATCH)`). Fallback is exact-token `instr` on `posts.tags` (same token list as MATCH). Steady-state gallery queries keep MATCH.
