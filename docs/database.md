...
8. **Default schedule:** `autoBackupInterval` defaults to `"never"` (opt-in). Changing the default is a separate product step after the backup→restore path is proven.
