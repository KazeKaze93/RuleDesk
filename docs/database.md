...
**Implementation:** `src/main/bootstrap-user-data.ts` runs before logger and `electron-store` so live paths resolve under `RuleDesk-Data`. On upgrade it copies light files (`backup-settings.json`, `logs/app.log`) from the Electron default product dir and from legacy `.rdcache` when targets are missing. `initializeDatabase` then moves `data.bin` via `migrateLegacyDatabase`: first from Electron-default `metadata.db` locations, then from `.rdcache/data.bin`. Wipe (`system:wipe-all-data`) deletes children of `RuleDesk-Data` only — it does **not** delete `RuleDesk-Backups`.
