...
- **Windows:** `%LOCALAPPDATA%\RuleDesk-Data\`
  - Database: `data.bin`
  - Logs: `logs\app.log`
  - Backup schedule sidecar: `backup-settings.json`
  - **Backups (sibling):** `%LOCALAPPDATA%\RuleDesk-Backups\`
- **macOS:** `~/Library/Application Support/RuleDesk-Data/` (backups: `~/Library/Application Support/RuleDesk-Backups/`)
- **Linux:** `~/.config/RuleDesk-Data/` (backups: `~/.config/RuleDesk-Backups/`)

**Implementation:** `src/main/bootstrap-user-data.ts` runs before logger and `electron-store` so live paths resolve under `RuleDesk-Data`. On upgrade it copies light files (`backup-settings.json`, `logs/app.log`, `Local State`) when targets are missing — **fresher `.rdcache` first**, Electron-default product dir only as fallback (copy is not rename; both sources may still exist on disk for long-time users, and first-write-wins would otherwise revive a stale pre-redirect `app.log`). `Local State` holds Chromium `os_crypt` material used by Electron `safeStorage`; without the matching file, API keys in a migrated `data.bin` cannot be decrypted. If the target still has no `data.bin` but a legacy dir still does (`data.bin` or ancient `metadata.db`), bootstrap **overwrites** a stale target `Local State` from that source (covers a failed earlier launch that already wrote a fresh profile key). Already-migrated installs that kept a wrong `Local State` are not repaired automatically — re-enter credentials in Settings → Account, or restore `Local State` from the leftover legacy folder before restarting. `initializeDatabase` then moves `data.bin` via `migrateLegacyDatabase`: first from Electron-default `metadata.db` locations, then from `.rdcache/data.bin` (DB migrate uses rename and already consumed the ancient source for most installs). Wipe (`system:wipe-all-data`) deletes children of `RuleDesk-Data` only — it does **not** delete `RuleDesk-Backups`.

**Downgrade limitation:** Older binaries do not know about newer data folders and may create an empty DB beside the real one. User-facing boundaries (same as README): do not run below **v17.0.0** after a `.rdcache`-era install; after the `RuleDesk-Data` relocate, do not run below **v18.1.0**. Prefer `RuleDesk-Backups` or support before continuing a rollback.
...
| `sync_status`     | TEXT (NOT NULL, DEFAULT 'idle')   | Current sync state (`idle`, `syncing`, `error`); written by `SyncService` |
| `last_error`      | TEXT (NULL)                       | Last provider/sync error message (`SyncService`); null when idle |
...
- Neutral `userData` path (`RuleDesk-Data`) for dev and packaged builds
...
2. **Backup Location:** Backups live under a dedicated directory **outside** live userData (so a wipe or cache cleaner cannot remove the live DB and every recovery snapshot together):
   - **Default path:** `<neutralRoot>/RuleDesk-Backups/` — sibling of `RuleDesk-Data` under the same root (`%LOCALAPPDATA%` on Windows, Electron `appData` on macOS/Linux; see `getNeutralDataRoot()` / `bootstrap-user-data.ts`). Name must not collide with `LEGACY_USER_DATA_DIR_NAMES` (e.g. legacy `RuleDesk/`) or `USER_DATA_DIR_NAME`.
   - **Live DB:** `data.bin` / `-wal` / `-shm` under `RuleDesk-Data/` (migrated from `.rdcache` on first launch of this build)
...
3. **One-time migrate on upgrade:** deferred startup (`scheduleDeferredStartupTasks`) moves leftover backup files (and matching `.settings.json` sidecars) from live userData and from legacy `.rdcache` into `RuleDesk-Backups` via `migrateBackupDirectory` + `moveFileWithExdevFallback`. Non-blocking, non-fatal on error; already-present target names are skipped (no overwrite). Next backup always writes to the new directory.
...
7. **Optional total-size cap (env):** If `BACKUP_RETENTION_MAX_TOTAL_MB` is set to a positive number, **manual** backup cleanup additionally prunes oldest files until total backup size is under the configured threshold. `runningTotal` only counts retained files. If the newest backup alone exceeds the cap, it is still kept so at least one backup always survives.
8. **Default schedule:** New installs (no `backup-settings.json` on disk before the first `BackupService` store construction) default `autoBackupInterval` to `"daily"`. Existing installs that already have the file keep their stored value — `conf`/`electron-store` merge does **not** silently upgrade `"never"` to `"daily"`. For those still on `"never"` who have never been asked, a one-time dismissible UI prompt (`maintenance:should-show-backup-prompt` / `maintenance:mark-backup-prompt-seen`, field `hasSeenAutoBackupPrompt`) offers daily backups; Enable / Not now / close all mark the prompt as seen (no silent flip, no repeat nagging).
9. **Restore dialog:** `defaultPath` opens in `getBackupDirectory()`.
...
- Live `data.bin` lives under `RuleDesk-Data/`; backup files live under sibling `RuleDesk-Backups/`. Leftover `.rdcache` is a migrate source only.
...
3. Copy the backup file to replace `data.bin` inside `RuleDesk-Data`
...
- **Table existence check:** `postsFtsTableExists(sqlite)` in `src/main/db/fts-table-check.ts` (schema introspection via `sqlite_master`; sqlite instance is passed in). `PlaylistController` and `PostsController` cache the result once in `setup()`. Missing table → Posts AI filter uses exact-token `instr` on `posts.tags`. Trigger DDL / rebuild stay in `fts-triggers.ts`.
- **Smart-playlist empty-guard / bulk-sync window:** Missing `posts_fts` → `sql\`1 = 0\``. When insert/update triggers are live, an empty content table (`SELECT 1 FROM posts_fts LIMIT 1` — safe only then, because content emptiness equals index emptiness) also returns `1 = 0`. While those triggers are dropped, `PlaylistController` does **not** use MATCH (include would fail-closed, exclude would fail-open, same class as AI-filter hide). Fallback is per-tag `instr` on `posts.tags` (AND includes / OR excludes); trailing `*` is a token-prefix `LIKE` on space-wrapped tags, not the AI-filter sanitizer. Steady-state smart playlists keep MATCH. No in-memory count cache.
...
- **Initial sync / hard-kill recovery:** `posts_fts_insert` and `posts_fts_update` are runtime-droppable together during initial sync / repair bulk upsert (insert = perf; update = correctness — `'delete'` on a never-indexed rowid corrupts the VTAB while insert is down). Restored via `ensureFtsTriggers` (`src/main/db/fts-triggers.ts`). After bulk upsert, `backfillArtistFtsIndex` runs `rebuildFtsIndex` (`VALUES('rebuild')`) — a blind artist-scoped `INSERT…SELECT` leaves stale FTS terms when repair conflicts update `posts.tags` while the update trigger is down (`'delete'` needs the previously indexed values, which are already overwritten). Hard-kill recovery calls `rebuildFtsIndex` directly. Both helpers live in `fts-triggers.ts`. `posts_fts_delete` stays live. The same `initializeDatabase` path then runs `resetStaleSyncingArtists`: `UPDATE artists SET sync_status = 'idle' WHERE sync_status = 'syncing'` only — `'error'`, `lastError`, and `lastSyncIncomplete` are not cleared.
- **AI filter / smart playlist during the bulk-sync window:** `PostsController` AI hide/only and `PlaylistController` smart-playlist tag conditions use FTS `MATCH` only while `areRuntimeDroppableFtsTriggersPresent` is true (real `sqlite_master` trigger DDL, not an FTS content probe and not `SyncService.isSyncing` — incremental sync is `isSyncing` with triggers still live). While insert/update are dropped, MATCH on never-indexed rows would fail-open hide/exclude (`NOT EXISTS(MATCH)`) and fail-closed include. AI fallback is exact-token `instr` on `posts.tags` (same token list as MATCH). Smart-playlist fallback is the same `instr` grammar for exact tags, plus token-prefix `LIKE` for trailing `*`. Steady-state gallery and playlist queries keep MATCH.
