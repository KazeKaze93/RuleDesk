...
**Downgrade limitation:** Older binaries do not know about newer data folders and may create an empty DB beside the real one. User-facing boundaries (same as README): do not run below **v17.0.0** after a `.rdcache`-era install; after the `RuleDesk-Data` relocate, do not run below **`<next release that includes RuleDesk-Data — fill in when tagging>`** (relocate is on `master`, not in any `v*` tag yet — replace the placeholder at release time). Prefer `RuleDesk-Backups` or support before continuing a rollback.
