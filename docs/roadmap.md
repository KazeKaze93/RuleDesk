...
| `fix/rule34-api-cdn-mp4-allowlist` | 🔄 this branch | Add `api-cdn-mp4.rule34.xxx` to Rule34 `allowedDomains` (CSP) and `cdnDomains` (video-proxy). Exact hostname match: `api-cdn.rule34.xxx` does not cover the MP4 CDN. Browse Source: All Videos/Images inject live-verified `video` / `-video` into `searchBooru` (Rule34 + Gelbooru); worker remains fallback on chip conflict. |
