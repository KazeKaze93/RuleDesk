...
| `fix/e2e-always-rebuild-on-ci` | ✅ merged | `tests/e2e/global-setup.ts`: always `npm run build` when `process.env.CI` is set; local `existsSync` skip retained. No mtime/hash (rejected — flaky on CI runners). |
...
| `chore/p3-test-suite-hygiene` | ✅ merged | [#159](https://github.com/KazeKaze93/RuleDesk/pull/159) — Coverage docs as file inventory (no stale case totals); filter-utils space-phrase tests rewritten as token-exact; startup.spec asserts Age Gate on a fresh profile. Dead space-containing `aiTags` in `hasAiGeneratedTag` (filter-utils **and** Browse worker) — closed in [#160](https://github.com/KazeKaze93/RuleDesk/pull/160). |
| `chore/remove-dead-ai-tag-phrases` | ✅ merged | [#160](https://github.com/KazeKaze93/RuleDesk/pull/160) — Removed unreachable `"ai generated"` / `"ai generation"` / `"ai generated content"` from `filter-utils.ts` and Browse worker. Live Rule34/Gelbooru autocomplete: space queries return `[]`; `_`/`-` tokens are real. Consecutive-token matching rejected (false positives on independent `generated`). |
| `fix/masonry-consolidate-virtuoso-item-classes` | ✅ merged | [#161](https://github.com/KazeKaze93/RuleDesk/pull/161) — Single `createVirtuosoGridFactories` for all five galleries; masonry items `w-full mb-4 break-inside-avoid`. Shared `useMasonryInfiniteScroll`. Removed Virtuoso tail-f (`atBottomStateChange` + length effect) from Browse — downward pagination is `endReached` only. |
| `fix/viewer-hasnextpage-predicate-off-by-boundary` | ✅ merged | [#162](https://github.com/KazeKaze93/RuleDesk/pull/162) — Playlist/Artist `openViewer` `hasNextPage` is the react-query flag, not `displayedCount < pages*50` / `allPosts.length < totalCount`. Browse already passed the flag. |
| `chore/provider-error-kind-not-text-based` | ✅ merged | [#163](https://github.com/KazeKaze93/RuleDesk/pull/163) — `throwProviderSearchIpcError` JSON-encodes `providerKind` into `Error.message`; parser no longer infers kind from `PROVIDER_SEARCH_USER_MESSAGES`. User copy is provider-agnostic (no hardcoded "Rule34"). |
| `fix/rule34-api-cdn-mp4-allowlist` | ✅ merged | [#164](https://github.com/KazeKaze93/RuleDesk/pull/164) — Add `api-cdn-mp4.rule34.xxx` to Rule34 `allowedDomains` (CSP) and `cdnDomains` (video-proxy). Exact hostname match: `api-cdn.rule34.xxx` does not cover the MP4 CDN. Browse Source: All Videos/Images inject live-verified `video` / `-video` into `searchBooru` (Rule34 + Gelbooru); worker remains fallback on chip conflict. |
| `chore/wire-appendQueueIds-viewer-open-close` | ✅ merged | [#165](https://github.com/KazeKaze93/RuleDesk/pull/165) — Masonry `onLoadMore` (all five) and local grid `endReached` (Favorites/Updates/Playlist) use gallery `handleLoadMore` (`appendQueueIds`). Browse/Artist grid still `useGalleryInfiniteScroll.handleEndReached`. |
| `chore/cdn-domain-runtime-guards` | ✅ merged | [#166](https://github.com/KazeKaze93/RuleDesk/pull/166) — Startup throw if `cdnDomains ⊈ allowedDomains`. Fetch-time `log.warn` once per hostname when `file_url` host is missing from `allowedDomains`, or is video and missing from `cdnDomains`. `RULE34_IMAGE_HOSTS` accepts `api-cdn-mp4.rule34.xxx` (not a rewrite target). |
...
| `feat/search-results-persistent-cache` | ✅ merged | [#167](https://github.com/KazeKaze93/RuleDesk/pull/167) — SQLite TTL cache for Browse `searchBooru` pages. Unbounded `beforePostId` keys → [planned row cap](#planned-product-work), not media LRU. |
...
| `feat/backup-default-opt-in-prompt` | — | 🔧 in progress | New installs default auto-backup to daily; existing `never` users get a one-time dismissible opt-in prompt (no silent flip) |
...
| **Engineering** | Ongoing tooling hygiene (`validate` green). Shared IPC validation is a single path (`BaseController.handle`); the former `createValidatedHandler` / `parseNoArgs` / `parseSingleArg` stack is gone (checked 2026-09-12). |
