...
| `chore/cdn-domain-runtime-guards` | 🔄 this branch | Startup throw if `cdnDomains ⊈ allowedDomains`. Fetch-time `log.warn` once per hostname when `file_url` host is missing from `allowedDomains`, or is video and missing from `cdnDomains`. `RULE34_IMAGE_HOSTS` accepts `api-cdn-mp4.rule34.xxx` (not a rewrite target). |
