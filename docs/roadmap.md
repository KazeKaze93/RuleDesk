...
| `fix/masonry-consolidate-virtuoso-item-classes` | 🟡 in progress | Single `createVirtuosoGridFactories` for all five galleries. Masonry items: `w-full mb-4 break-inside-avoid` (CSS columns). Removed leftover flex-wrap `w-[calc(...)]` from factory Item. `pb-44` unified. `PlaylistVirtuosoComponents` keeps only `SortablePostCard`. Masonry load-more: `useMasonryInfiniteScroll`. Removed Virtuoso tail-f (`atBottomStateChange` + `allPosts.length` effect) from `useGalleryInfiniteScroll` / Browse — downward pagination is `endReached` only. |
