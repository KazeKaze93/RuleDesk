...
| **E2E rebuild-skip** | `tests/e2e/global-setup.ts` skips `npm run build` when `out/main/main.cjs` exists (`existsSync` only). Replace with mtime/hash of sources vs bundle (or always rebuild on CI) so e2e cannot silently run yesterday’s main. Separate small branch; not a blocker. |
