...
| `chore/wire-appendQueueIds-viewer-open-close` | 🔄 this branch | Masonry `onLoadMore` (all five) and local grid `endReached` (Favorites/Updates/Playlist) use gallery `handleLoadMore` (`appendQueueIds`). Browse/Artist grid still `useGalleryInfiniteScroll.handleEndReached`. |
