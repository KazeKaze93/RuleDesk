...
| **Gallery layout (P0-class drift)** | `src/renderer/components/gallery/virtuoso-factories.tsx` is the only GridContainer source the P2 RTL suite actually asserts (grid-template-columns / `columns-N` / flex-width masonry items, `pb-44`). `ArtistGallery.tsx` and `PlaylistVirtuosoComponents.tsx` still ship their own copies: masonry uses `break-inside-avoid` + `pb-32` (Artist) / `pb-44` (Playlist) instead of the factory flex-width + `pb-44`. Browse / Favorites / Updates go through the factory; Artist and Playlist galleries can render a different grid (padding and masonry column behavior). Not unified in `fix/p2-test-suite-reliability` — either a real visual split or leftover dupes that should call `createVirtuosoGridFactories`. |
...
| **AI tag phrases (P3)** | `hasAiGeneratedTag` in `src/renderer/lib/filter-utils.ts` (Favorites / Updates / PlaylistGallery) **and** the copy in `src/renderer/workers/data-processor.worker.ts` (Browse remote worker) both list three space-containing `aiTags` / `AI_TAG_PATTERNS` entries: `"ai generated"`, `"ai generation"`, `"ai generated content"`. After `tagArray = tags.split(/\s+/)` those strings can never `includes()`-match a token. Either they are leftover duplicates (real booru tags use `_`/`-`; space variants are dead) **or** matching needs a different algorithm (consecutive tokens, not exact `includes`). **Decide before fixing** — may be harmless dead code if live booru tags never contain spaces. Found in `chore/p3-test-suite-hygiene`; tests now assert token-exact behavior; prod lists not changed. |
