...
| `chore/provider-error-kind-not-text-based` | 🔄 this branch | `throwProviderSearchIpcError` JSON-encodes `providerKind` into `Error.message`; parser no longer infers kind from `PROVIDER_SEARCH_USER_MESSAGES`. User copy is provider-agnostic (no hardcoded "Rule34"). |
