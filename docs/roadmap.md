...
| `chore/p3-test-suite-hygiene` | ✅ merged | [#159](https://github.com/KazeKaze93/RuleDesk/pull/159) — Coverage docs as file inventory (no stale case totals); filter-utils space-phrase tests rewritten as token-exact; startup.spec asserts Age Gate on a fresh profile. Dead space-containing `aiTags` in `hasAiGeneratedTag` (filter-utils **and** Browse worker) — closed in `chore/remove-dead-ai-tag-phrases`. |
| `chore/remove-dead-ai-tag-phrases` | 🟡 in progress | Remove unreachable `"ai generated"` / `"ai generation"` / `"ai generated content"` from `filter-utils.ts` and Browse worker. Live Rule34/Gelbooru autocomplete: space queries return `[]`; `_`/`-` tokens are real. Consecutive-token matching rejected (false positives on independent `generated`). |
