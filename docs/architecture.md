...
12. ✅ **Backup/Restore:** Manual/auto database backup (`VACUUM INTO` → `RuleDesk-Backups/`) with integrity checks, retention, and read-only orphan detection
...
- ✅ **Database Backup / Restore System** - ✅ **COMPLETED:** `VACUUM INTO` snapshots under `RuleDesk-Backups/`, integrity checks, retention, read-only orphan detection

### Future Considerations

1. **tag-combination subscriptions feature/table:** **Closed — superseded by smart playlists (`is_smart`/`query_json`); no separate entity built (2026-09-12 backlog audit).** Distinct from the shipped Browse Source Subscriptions filter (`sinceTracking`). See [Closed by design](./roadmap.md#closed-by-design-not-backlog).
