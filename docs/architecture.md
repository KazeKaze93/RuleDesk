...
12. **Browse** (`src/renderer/components/pages/Browse.tsx`, `SearchController.search`, `src/renderer/utils/react-query-cache.ts`)
   ...
   - **Client-side filter/sort (remote only):** `useWorkerFilteredPosts` runs only when Source is **All** and filters are non-default (`usesDefaultRemoteFilters` skip: no tags + AI All + Media All). The worker applies AI/media + sort to live `searchBooru` pages; it does not implement Source. Worker output is mapped back via `mapWorkerPostToPost()` in `src/renderer/lib/map-worker-post.ts` (preserves `mediaType`, `viewCount`, `lastViewedAt`; infers `mediaType` from `fileUrl` when missing).
   - Filter config is debounced (~250 ms) on the remote worker path; raw post pages are not debounced (scroll/load latency). Browse `queryKey` is `buildBrowseSearchQueryKey({ tags, source, aiFilter, mediaType, sortOrder })` — isomorphic with viewer cache lookup.
